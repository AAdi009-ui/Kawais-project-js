    ...
    async def extraowner (self ,ctx ,option :str =None ,user :discord .Member =None ):
        ...
        if option .lower ()=='set':
            if user is None or user .bot :
                embed =discord .Embed (description ="Please Provide a Valid User Mention or ID to Set as Extra Owner!",
                color =0x000000 
                )
                await ctx .reply (embed =embed )
                return 
            ...
            if view .value is None :
                ...
                await message .edit (content ="<:icon_cross:1372375094336425986> Action cancelled.",embed =None ,view =None )
            ...
            if not row :
                embed =discord .Embed (description ="No extra owner has been designated for this guild.",
                color =0x000000 
                )
                await ctx .reply (embed =embed )
            else :
                ...
                if view .value is None :
                    ...
                    await self .db .commit ()
                    embed =discord .Embed (title ="<:strelizia_tick:1227866641027698792> Success",
                    description ="Disabled Extraowner Configuration!",
                    color =0x000000 
                    )
                    await message .edit (embed =embed ,view =None )
                else :
                    await message .edit (content ="<:icon_cross:1372375094336425986> Action canceled.",embed =None ,view =None )
            ...
            if not row :
                embed =discord .Embed (description ="No extra owner is currently assigned.",
                color =0x000000 
                )
                await ctx .reply (embed =embed )
            else :
