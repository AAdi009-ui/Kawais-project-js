...
class Customrole (commands .Cog ):
        ...
        async with aiosqlite .connect (DATABASE_PATH )as db :
            async with db .execute (f"SELECT reqrole, {role_type} FROM roles WHERE guild_id = ?",(context .guild .id ,))as cursor :
                ...
                if data :
                    ...
                    if not role_id :
                        embed =discord .Embed (
                        title ="<:icon_cross:1372375094336425986> Role Not Configured",
                        description =f"{role_type.capitalize()} role is not configured in {context.guild.name}.\nPlease use `setup {role_type} @role` to configure it first.",
                        color =0x000000 
                        )
                    ...
                    if not role :
                        embed =discord .Embed (
                        title ="<:icon_cross:1372375094336425986> Role Not Found",
                        description =f"The configured {role_type} role no longer exists in {context.guild.name}.\nPlease reconfigure it using `setup {role_type} @role`.",
                        color =0x000000 
                        )
                        await context .reply (embed =embed )
                        return 
                    ...
                    if reqrole_id and not reqrole :
                        embed =discord .Embed (
                        title ="<:icon_danger:1373170993236803688> Required Role Missing",
                        description =f"The required role for using custom roles no longer exists in {context.guild.name}.\nPlease reconfigure it using `setup reqrole @role`.",
                        color =0x000000 
                        )
                        await context .reply (embed =embed )
                        return 
                    ...
                    try :
                        if role in member .roles :
                            ...
                            embed =discord .Embed (
                            title ="<:icon_tick:1372375089668161597> Success",
                            description =f"**Given** {role.mention} to {member.mention}",
                            color =0x000000 
                            )
                        await context .reply (embed =embed )
                    except discord .Forbidden :
                        embed =discord .Embed (
                        title ="<:icon_cross:1372375094336425986> Permission Error",
                        description ="I don't have permission to manage this role. Please check my role hierarchy and permissions.",
                        color =0x000000 
                        )
                        await context .reply (embed =embed )
                    except Exception as e :
                        embed =discord .Embed (
                        title ="<:icon_cross:1372375094336425986> Error",
                        description =f"An error occurred while managing the role: {str(e)}",
                        color =0x000000 
                        )
                        await context .reply (embed =embed )
                else :
                    embed =discord .Embed (
                    title ="<:icon_cross:1372375094336425986> Setup Required",
                    description =f"Role configuration is not set up in {context.guild.name}.\nPlease use `setup {role_type} @role` to configure it first.",
                    color =0x000000 
                    )
                    await context .reply (embed =embed )
    ...
    async def config (self ,context :Context )->None :
        ...
        if role_data :
            ...
            embed =discord .Embed (description ="No custom role configuration found in this Guild.",
            color =0x000000 
            )
        ...
        async with aiosqlite .connect (DATABASE_PATH )as db :
            ...
            async with db .execute ("SELECT name FROM custom_roles WHERE guild_id = ?",(context .guild .id ,))as cursor :
                ...
                if any (name ==row [0 ]for row in existing_role ):
                    embed =discord .Embed (description =f"A custom role command with the name `{name}` already exists in this guild. Remove it before creating a new one.",
                    color =0x000000 
                    )
                    await context .reply (embed =embed )
                    return 
    ...
    async def delete (self ,context :Context ,name :str )->None :
        ...
        if not existing_role :
            embed =discord .Embed (description =f"No custom role command with the name `{name}` was found in this guild.",
            color =0x000000 
            )
            await context .reply (embed =embed )
            return 
        ...
        if context .author ==context .guild .owner or context .author .top_role .position >context .guild .me .top_role .position :
            ...
            if role_data :
                ...
                async with aiosqlite .connect (DATABASE_PATH )as db :
                    ...
                    description =f"Deleted All Custom Role commands <:strelizia_tick:1227866641027698792>\n\n**Removed Roles:**\n"+"\n".join (removed_roles )if removed_roles else "No roles were previously set.",
                    color =0x000000 
                    )
    ...
    async def on_message (self ,message :discord .Message ):
        ...
        if result :
            ...
            if reqrole not in message .author .roles :
                await message .channel .send (embed =discord .Embed (description =f"<:icon_danger:1373170993236803688> You need the {reqrole.mention} role to use this command.",color =0x000000 ))
                return 
