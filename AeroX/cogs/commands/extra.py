...
class RoleInfoView (View ):
  ...
  @discord .ui .button (label ='Show Permissions',emoji ="<:admin:1373949463814738002>",style =discord .ButtonStyle .secondary )
...
class OverwritesView (View ):
  ...
  async def show_overwrites (self ,interaction :discord .Interaction ,button :Button ):
      ...
      for target ,perms in self .channel .overwrites .items ():
          ...
          "\n".join (f"  * **{perm}:** {'<:icon_tick:1372375089668161597>' if value else '<:icon_cross:1372375094336425986>' if value is False else '⛔'}"for perm ,value in permissions .items ()))
      ...
      embed .set_footer (text ="<:icon_tick:1372375089668161597>= Allowed, <:icon_cross:1372375094336425986> = Denied, ⛔ = None")
      await interaction .response .send_message (embed =embed ,ephemeral =True )
...
class Extra (commands .Cog ):
  ...
  async def serverinfo (self ,ctx ):
        ...
        if ctx .guild .features :
            features ="\n".join ([f"<:icon_tick:1372375089668161597>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}"for feature in ctx .guild .features ])
            embed .add_field (
            name ="**__Features__**",
        ...
        value =f"Level: {ctx.guild.premium_tier} [<:booste:1373949048758997034> {ctx.guild.premium_subscription_count} boosts]",
        inline =False 
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:icon_tick:1372375089668161597>Yes' if member.bot else '<:icon_cross:1372375094336425986> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts (self ,ctx ):
    await ctx .send (
    embed =discord .Embed (title =f"<:booste:1373949048758997034> Boosts Count Of {ctx.guild.name}",
  ...
  async def list_admin (self ,ctx ):
    ...
    paginator =Paginator (source =DescriptionEmbedPaginator (
    entries =entries ,
    title =f"Adminsin {guild.name} - {admins}",
    description ="",
    per_page =10 ),
    ctx =ctx )
    await paginator .paginate ()
  ...
  async def report (self ,ctx ,*,bug ):
    ...
    confirm_embed =discord .Embed (title ="<:icon_tick:1372375089668161597>Bug Reported",
