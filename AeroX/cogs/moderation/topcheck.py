...
class TopCheck (commands .Cog ):
    ...
    async def topcheck_enable (self ,ctx ):
        if ctx .author .id !=ctx .guild .owner_id :
            return await ctx .reply ("<:icon_cross:1372375094336425986> Only the **Server Owner** can enable topcheck.")
        if await self .is_topcheck_enabled (ctx .guild .id ):
            return await ctx .reply ("<:icon_cross:1372375094336425986> Topcheck is already enabled for this server.")
        await self .enable_topcheck (ctx .guild .id )
        await ctx .reply ("<:icon_tick:1372375089668161597> Topcheck has been Successfully enabled for this server.")
    ...
    async def topcheck_disable (self ,ctx ):
        ...
        if not await self .is_topcheck_enabled (ctx .guild .id ):
            return await ctx .reply ("<:icon_cross:1372375094336425986> Topcheck is not enabled for this server.")
