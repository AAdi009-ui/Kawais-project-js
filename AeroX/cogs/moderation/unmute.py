...
class MuteReasonModal (ui .Modal ):
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        ...
        try :
            await self .user .send (f"<:warning:1372989665115901994> You have been muted in **{interaction.guild.name}** for {time_str}. Reason: {reason}")
            dm_status ="Yes"
        except discord .Forbidden :
            dm_status ="No"
...
class Unmute (commands .Cog ):
    ...
    async def unmute (self ,ctx ,user :discord .Member ):
        ...
        try :
            ...
            try :
                await user .send (f"<:icon_tick:1372375089668161597> You have been unmuted in **{ctx.guild.name}**.")
                dm_status ="Yes"
            except discord .Forbidden :
                dm_status ="No"
            except discord .HTTPException :
